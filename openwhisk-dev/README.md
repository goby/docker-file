...
OpenWhisk-dev is a image based on `java:8-jdk` for setuping [openwhisk](#what-is-openwhisk) development environment as quickly as possible.
This image include:
* Base image: `java:8-jdk`
* Kernel: 4.4.0
...
* `latest`, `jdk-8`([Dockerfile](https://github.com/goby/docker-file/blob/openwhisk-dev-jdk-8/openwhisk-dev/Dockerfile))

## How to use

Goto path to openwhisk source root, and run following:

```bash
# the docker tool and docker endpoint should mount into container to run gradle distDocker
docker run -it -v $PWD:/home/whisk/dev/openwhisk --user root -v /usr/bin/docker:/usr/bin/docker -v /var/run/docker.sock:/var/run/docker.sock openwhisk-dev:jdk-8 bash
# in the docker
# you may need change owner of the source code
# running gradle task
gradle distDocker
